...
import secrets
import sqlite3
from models import (RegraNegocio, texto, obter, papel, Hackathon, Equipe,
...
class Sistema:
    ...
    def cadastrar(self, nome, email, senha, *, organizador=False):
        ...
        if '@' not in email or not all(email.split('@')) or email.count('@') != 1:
            raise RegraNegocio('E-mail inválido.')
        if len(senha) < 6:
            raise RegraNegocio('A senha precisa ter pelo menos 6 caracteres.')
        try:
            with self.db:
            ...
            raise RegraNegocio('E-mail já cadastrado.') from None
    ...
    def login(self, email, senha):
        ...
        if not u or not hmac.compare_digest(u['senha'], senha_hash(senha, u['senha'].split(':')[0])):
            raise RegraNegocio('E-mail ou senha incorretos.')
        return u['id']
    ...
    def hackathons(self):
        return [Hackathon(**dict(r)).vincular(self.db) for r in self.db.execute('SELECT * FROM hackathons ORDER BY id')]
    ...
    def organizador(self, u):
        ...
        return Organizador(u, usuario['nome'], usuario['email']).vincular(self.db)
    ...
    def atuacao(self, u, h, esperado=None):
        ...
        return classe(u, usuario['nome'], usuario['email'], h).vincular(self.db)
